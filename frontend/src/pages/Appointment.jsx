...
const Appointment = () => {
    ...
    return docInfo ? (
        <div>
            ...
            {/* Reviews */}
            <div className='sm:ml-72 sm:pl-4 mt-2'>
                <div className='flex items-center gap-2'>
                    <p className='text-lg font-medium text-gray-700'>Patient Reviews</p>
                    <p className='text-sm text-gray-500'>({totalReviews})</p>
                </div>
                {totalReviews > 0 && (
                    <p className='text-sm text-gray-600 mt-1'>Average rating: <span className='font-medium'>{averageRating} / 5</span></p>
                )}
                {reviews.length > 0 ? (
                        ...
                        {reviews.map((item) => (
                            <div key={item._id} className='border rounded-lg p-4 bg-white'>
                                <div className='flex items-center justify-between gap-3'>
                                    <p className='font-medium text-gray-700'>{item.userData?.name || 'User'}</p>
                                    <p className='text-sm text-amber-500'>{'★'.repeat(item.rating)}{'☆'.repeat(5 - item.rating)}</p>
                                </div>
                                {item.review && <p className='text-sm text-gray-600 mt-2'>{item.review}</p>}
                            </div>
                        ))}
                    </div>
                ) : (
