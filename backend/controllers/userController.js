...
// API to select cash payment for appointment
const paymentCash = async (req, res) => {
    try {
        ...
        if (appointmentData.isCompleted) {
            return res.json({ success: false, message: 'Completed appointment cannot be updated' })
        }

        await appointmentModel.findByIdAndUpdate(appointmentId, { paymentType: 'cash' })
...
const verifyRazorpay = async (req, res) => {
    try {
        ...
        if (orderInfo.status === 'paid') {
            await appointmentModel.findByIdAndUpdate(orderInfo.receipt, { payment: true, paymentType: 'online' })
...
// API to add review for completed appointment
const addReview = async (req, res) => {
    try {
        ...
        if (!appointmentData.isCompleted || appointmentData.cancelled) {
            return res.json({ success: false, message: 'Review can only be added for completed appointments' })
        }
