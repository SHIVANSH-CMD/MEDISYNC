...
// API to get public reviews of a doctor
const doctorReviews = async (req, res) => {
    try {
        const { docId } = req.params

        const reviewData = await appointmentModel.find({
            docId,
            reviewed: true,
            cancelled: false
        }).select('rating review userData date').sort({ date: -1 })

        const totalReviews = reviewData.length
        const averageRating = totalReviews
            ? Number((reviewData.reduce((sum, item) => sum + item.rating, 0) / totalReviews).toFixed(1))
            : 0

        res.json({ success: true, reviews: reviewData, totalReviews, averageRating })
    } catch (error) {
        console.log(error)
        res.json({ success: false, message: error.message })
    }
